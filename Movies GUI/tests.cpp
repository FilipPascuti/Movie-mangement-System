...
void test_all()
{
	...
	/*Movie m{ "asdf", "asdfff", 1000, 2222222, "asdf.mp4" };
	Movie m2{ "asdf", "mare", 11111, 555, "par.mp4" };

	Dynamic_vector<Movie> movies;
	movies.add_element(m);
	movies.add_element(m2);

	Dynamic_vector<Movie>::iterator iterator = movies.begin();*/

	/*Dynamic_vector<int> vector{ 2 };
	vector.add_element(10);
	vector.add_element(20);
	assert(vector.get_length() == 2);
	assert(vector[1] == 20);
	vector[1] = 40;
	assert(vector[1] == 40);


	Dynamic_vector<int> vector2{ vector };

	assert(vector2.get_length() == 2);

	Dynamic_vector<int> vector3;
	vector3.add_element(10);
	vector3 = vector3;
	assert(vector3[0] == 10);

	Dynamic_vector<int*> vector4{};
	assert(vector4.get_length() == 0);

	Dynamic_vector<int>::iterator iter = vector.begin();
	assert(*iter == 10);
	assert(iter != vector.end());
	++iter;
	assert(*iter == 40);

	vector.delete_element(10);*/

	/*Dynamic_vector<Movie> array;
	Movie m{ "asdf", "asdfff", 1000, 2222222, "asdf.mp4" };
	Movie m2{ "asdf", "mare", 11111, 555, "par.mp4" };
	array.add_element(m);
	array.add_element(m2);

	for (auto iterator = array.begin(); iterator != array.end(); iterator++) {
		cout << (*iterator).print_format() << "    ";
	}*/
	...
	//m.set_genre("new genre");
	//cout << m.get_genre()<< endl;

	/*movies.add_element(m);
	cout << movies[0].get_genre()<< endl;

	movies.delete_element(m);
	assert(movies.get_length() == 0);


	movies.update_element(m2);
	assert(movies.get_length() == 0);
	cout << movies[0].get_trailer() << endl;*/
	
	/*Repository repo;
	repo.add_entity(m);
	assert(repo.get_length() == 1);

	Dynamic_vector<Movie> movies = repo.get_all();
	cout << movies[0].get_genre() << endl;

	repo.delete_entity(m);
	assert(repo.get_length() == 0);
	
	repo.update_entity(m2);
	assert(repo.get_length() == 1);*/
	
	/*Service service;

	service.service_add_movie("asdf", "asdfff", 1000, 2222222, "asdf.mp4");
	Dynamic_vector<Movie> movies = service.get_all();
	...
	service.service_update_movie("asdf", "mare", 11111, 555, "par.mp4");
	movies = service.get_all();
	cout << movies[0].get_trailer() << endl;*/
	
	/*vector<string> parameters;
	vector<string> parameters1;
	string str = "title, genre, 2000, 11, trailer";
	string word = "";
	for (auto x : str)
	{
		if (x == ' ' || x == ',')
