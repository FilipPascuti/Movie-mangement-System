...
{
	/*this->iterator_for_printing = this->repository->get_iterator_begin();
	Movie current_movie = *this->iterator_for_printing;
	iterator_for_printing++;
...
{
	/*Movie current_movie;
	if (this->iterator_for_printing != this->repository->get_iterator_end()) {
		current_movie = *this->iterator_for_printing;
		iterator_for_printing++;
	}
	else {
		iterator_for_printing = this->repository->get_iterator_begin();
		current_movie = *this->iterator_for_printing;
		iterator_for_printing++;
	}*/
	Movie current_movie;
...
std::vector<Movie> Service::get_filtered_movies(string genre, int minimum_number_of_likes)
{
	...
	//std::partition(filtered_movies.begin(), filtered_movies.end(), [genre, minimum_number_of_likes](Movie movie) {return movie.get_genre() == genre && minimum_number_of_likes < movie.get_number_of_likes(); });	
...
void Service::initialize_repository(string filename)
{
	...
	//fileLocation D:\Facultate\Anul 1\Semestrul 2\OOP\Lab 5\Lab 5\Lab 5\movies.txt
}
